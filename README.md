...
- node-controller подтягивает актуальный `users.list` и server lists из панели в локальный `FPTN_CONFIG_DIR`, чтобы новые пользователи не терялись между панелью и нодой.
- для прод-узлов sync FPTN-конфига теперь идёт отдельно от heartbeat, поэтому добавление/удаление пользователя доходит до ноды автоматически и быстро.
